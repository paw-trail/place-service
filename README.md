...
place_db
  place                 19,501행     장소 본체
  place_source_link     22,914행     어느 소스의 어느 레코드에서 왔는지
  place_facility        14,404행     주차 · 산책로 · 놀이터 · 예약
  place_pending_update               잠긴 장소에서 수집이 발견한 값
  place_source_detach                관리자가 잘못 묶인 것을 떼어낸 기록
  + outbox                           공통 대역
```

**연결이 장소보다 많습니다.** 소스 넷이 같은 곳을 가리켜 합쳐진 곳이 3,403군데 있습니다.
...
**① 소스가 넷이고 같은 곳이 여러 번 들어옵니다**

```
PET_TOUR      한국관광공사 반려동물 동반여행    1,079건
GOCAMPING     한국관광공사 고캠핑             2,985건
CULTURE_CSV   한국문화정보원 문화시설         13,399건
MOIS_VET      행정안전부 동물병원 인허가       5,451건
                                        ─────────
                                    합계  22,914건  →  장소 19,501곳
```

> ⛔**`MOIS_VET` 만 오는 길이 다릅니다.** 다른 셋은 `ingest` 가 원본을 담아 둔 뒤 넘기는데
> 이 소스는 파일을 읽는 자리에서 바로 옵니다. 받는 쪽에서는 차이가 없습니다.
> 같은 `POST /internal/places/bulk` 로 들어옵니다.
...
한국관광공사     고캠핑                    2,985건
한국문화정보원   반려동물 동반 문화시설     13,399건
행정안전부       동물병원 인허가            5,451건
                                     ─────────
                                합계  22,914건
```
...
> **행정안전부 것은 성격이 조금 다릅니다.** 동반 조건이 아니라 인허가 대장입니다.
> 그 소스를 쓰는 이유는 **폐업한 병원을 가려내기 위해서**입니다.
> 문화정보원 자료는 2022년에 만들어져 그 사이 문을 닫은 곳이 섞여 있습니다.

---

### 2-2. 실제로 이렇게 생겼습니다

문암생태공원 한 곳을 소스 셋이 각각 이렇게 말합니다. 그 장소에는 행정안전부 자료가 없습니다.
...
| 실물 | 22,914건을 두 번 보냈는데 장소 수가 한 건도 안 늘었습니다 |
...
후보를 300m 반경으로 모음
   │
   └─ 그중 이름이 같은 것마다
        ├─ 지오코딩이 안 낀 것    →  100m 안이면 합침
        ├─ 한쪽이 지오코딩 좌표   →  300m 안이면 합침
        └─ 둘 다 지오코딩         →  ⛔합치지 않음
```

**좌표 출처가 셋입니다.** 조합마다 기준이 다릅니다.

| 조합 | 임계값 | 왜 |
|---|---|---|
| 원본 · 원본 | **100m** | 소스마다 대표 좌표를 다르게 잡습니다. 건물 중심을 쓰는 곳과 입구를 쓰는 곳이 있어 주소와 이름이 같은데 좌표만 100m 가까이 떨어진 쌍이 실제로 있습니다 |
| 원본 · 변환 | **100m** | 변환 좌표가 원본만큼 정확합니다. 아래를 보십시오 |
| 변환 · 변환 | **100m** | 같습니다 |
| 지오코딩이 끼면 | **300m** | 주소를 좌표로 바꾼 값이라 원본보다 오차가 큽니다 |
| 지오코딩끼리 | ⛔합치지 않음 | 둘 다 주소에서 만든 값이라 오차가 겹칩니다 |
| 후보 반경 | **300m** | 넓은 쪽으로 고정합니다 |

---

**변환 좌표를 원본과 같게 본 근거는 실측입니다.**

행정안전부 자료만 위경도가 아니라 미터 단위 평면 좌표(`EPSG:5174`)로 옵니다.
`ingest` 가 옮겨서 보내는데 그 값이 얼마나 정확한지를 재 보았습니다.

```
옮긴 5,258건 가운데 도로명주소가 같은 기존 장소와 맞은 1,783쌍
...
**90분위 20m 는 원본끼리도 나는 차이입니다.** 지오코딩 취급을 할 이유가 없습니다.

⛔**100m 를 넘긴 61건은 변환 오차가 아닙니다.** 도로명주소 하나가 넓은 부지를 가리키는 곳입니다.

```
에버랜드 동물병원         1,363m
국립생태원동물병원          918m
서울대학교 수의과대학 동물병원  665m
경상국립대학교 동물의료원      529m
```

한쪽은 정문 좌표를, 다른 쪽은 건물 좌표를 찍은 것입니다.
300m 로 늘려도 이들은 여전히 밖이고, 대신 도시에서 300m 안의 다른 병원을 붙일 위험이 생깁니다.
**거짓 병합이 거짓 분리보다 훨씬 위험하다**는 기준에 걸립니다.

> **변환끼리도 100m 로 봅니다.** 지오코딩끼리를 막는 근거는
> 둘 다 주소에서 만든 값이라 오차가 겹친다는 것인데,
> 변환 좌표는 소스가 준 측량값을 좌표계만 옮긴 것이라 그 근거가 성립하지 않습니다.

> ⚠**신뢰도도 원본과 같은 값을 씁니다.**
> 그 값은 검증할 때 낮은 것부터 뽑아 보기 위한 것인데,
> 실측이 원본만큼 정확하다고 말하는데 값만 낮추면 헛되이 위로 올라옵니다.
...
| 소스 | 넘긴 것 | 붙은 것 |
|---|---|---|
| `CULTURE_CSV` | 13,399 | 13,399 |
| `MOIS_VET` | 5,474 | **5,451** |
| `GOCAMPING` | 2,993 | **2,985** |
| `PET_TOUR` | 1,079 | 1,079 |

차이 나는 31건이 [3-6](#3-6-⑤-못-찾으면-새로-만듭니다) 의 건너뛴 것입니다.
좌표가 없고 지오코딩도 실패해 장소를 만들지 못한 것들입니다.

---

**어떻게 붙었는지도 남습니다.** `MOIS_VET` 5,451건을 보면 이렇습니다.

| 방법 | 건수 | |
|---|---|---|
| `PRIMARY` | 2,175 | 어디에도 안 붙어 새로 만들어진 것 |
| `ADDRESS` | 1,735 | 정규화 주소와 이름이 같아 붙은 것 |
| `COORD_ORIGINAL` | 1,470 | ⛔좌표로 붙은 것. 100m 기준입니다 |
| `COORD_GEOCODED` | 71 | 좌표가 없어 지오코딩한 뒤 붙은 것 |

⛔**`COORD_ORIGINAL` 1,470건이 변환 좌표를 원본과 같게 본 결과입니다.**
그 조합을 막아 두었다면 1,470건이 전부 새 장소가 되어,
같은 동물병원이 문화정보원 판과 행정안전부 판으로 두 번 떴을 것입니다.

---

**연결이 몇 개씩 붙었는지도 볼 수 있습니다.**
...
coord_source
  ...
  CONVERTED    다른 좌표계로 온 것을 위경도로 옮긴 것
...
```
ORIGINAL    17,323
CONVERTED    2,076     행정안전부 동물병원
GEOCODED       102     좌표가 없어 주소로 만든 것
```

> **`CONVERTED` 는 `ingest` 가 붙여 보냅니다.** 행정안전부 자료만 미터 단위 평면 좌표로 오는데
> 이쪽에 그대로 보내면 위경도 범위 검사에 걸려 "쓸 수 없는 좌표" 가 됩니다.
> 그러면 소스가 준 측량 좌표가 버려지고 전부 지오코딩으로 넘어갑니다.

> ⚠**합칠 상대의 좌표를 물려받은 것은 따로 표시하지 않습니다.**
> 물려받은 값도 결국 그 상대의 출처를 그대로 씁니다.
...
place  (19,501)
  │
  ├──< place_source_link  (22,914)     어느 소스에서 왔는지.  한 장소에 여럿
  │
  ├──< place_facility     (14,404)     주차 · 산책로 · 놀이터 · 예약
  │
...
⛔**연결이 장소보다 많습니다.** 소스 넷이 같은 곳을 가리켜 합쳐진 곳이 3,403군데입니다.
...
```
적재본 19,501행을 잃으면 다시 채우는 데 문제가 있음
```
...
**행정안전부 자료를 쓰는 이유의 절반이 아직 안 풀렸습니다.**

```
쓰는 이유    ① 빠진 병원을 채운다        ✅됨.  2,175곳이 새로 들어옴
            ② 폐업한 병원을 가려낸다     ⬜아직 안 함

⛔단순 비교로는 안 됨
   "행정안전부 영업중 목록에 없다" 가 "폐업했다" 를 뜻하지 않음
   두 소스가 상당 부분 겹치지 않아 애초에 그쪽에 없던 곳이 많음

정하려면    어느 소스에만 있을 때 폐업으로 볼지
            status 를 CLOSED 로 바꾸는 것이라 되돌리기가 어려움
```
...
| **변환 좌표** | 다른 좌표계로 온 것을 위경도로 옮긴 것. `ingest` 가 옮겨 보냅니다 |
