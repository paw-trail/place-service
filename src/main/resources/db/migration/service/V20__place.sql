...
(
    ...
    --
    -- 폭이 200 인 이유입니다.
    -- 적재본 전수 실측에서 가장 긴 값이 52 자였고 100 자를 넘는 행은 없었습니다.
    -- 그런데도 100 이 아니라 200 으로 둡니다.
    --
    -- 실측이 상한을 보장하지 않기 때문입니다.
    -- 문화정보원 CSV 는 판이 바뀌고 시설명 칸에 설명 문장이 들어간 행이 이미 있습니다.
    -- 가장 긴 값이 "유일동물병원 산업동물에 대한 출장진료를 전문으로 함.|..." 이었습니다.
    --
    -- 넘쳤을 때의 대가도 큽니다.
    -- ingest 가 청크로 적재하는 중에 삽입이 실패하면 그 청크가 통째로 롤백되어 수집이 멈춥니다.
    -- 값을 잘라 담는 것은 답이 아닙니다.
    -- 잘리면 uq_place_source 가 다른 장소를 같은 것으로 보아 재수집 멱등이 깨집니다.
    --
    -- 시설명과 지번 주소를 이은 값이라 두 컬럼 폭의 합보다 클 수 없으므로
    -- 그보다 크게 잡을 근거는 없습니다.
