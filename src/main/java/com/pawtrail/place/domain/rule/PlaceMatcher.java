...
public final class PlaceMatcher {
     ...
     *
     * 변환 좌표는 원본과 같은 100m 를 씁니다.
     * 행정안전부 동물병원 5,258 건을 EPSG:5174 에서 4326 으로 옮긴 뒤
     * 도로명주소가 같은 기존 장소와 거리를 재어 정한 값입니다.
     * 맞은 1,783 쌍의 중앙값이 3m 이고 90 분위가 20m 로 원본끼리도 나는 차이였습니다.
     * 한국 범위를 벗어난 좌표는 한 건도 없어 좌표계 자체가 맞다는 것도 확인했습니다.
     *
     * 100m 를 넘긴 61 건은 변환 오차가 아닙니다.
     * 에버랜드와 국립생태원처럼 도로명주소 하나가 넓은 부지를 가리키는 곳이라
     * 한쪽은 정문을, 다른 쪽은 건물을 찍은 것입니다.
     * 300m 로 늘려도 이들은 여전히 밖이고 도시에서 다른 장소를 붙일 위험만 커집니다.
     *
     ...
     *
     * 변환 좌표가 낀 것도 원본과 같은 값을 씁니다.
     * 실측이 원본만큼 정확하다고 말하는데 값만 낮추면
     * 낮은 것부터 뽑아 보는 이 쓰임에서 헛되이 위로 올라옵니다.
     *
